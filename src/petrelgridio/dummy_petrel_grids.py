...
def various_dirty_checks_to_be_cleaned():
    ...
    # print(collect_pillars_nodes(cube_grid))
    two_cubes = np.array([cube, cube + t(1, 0, 0.5)], dtype=cube.dtype)
    two_cubes_grid = two_cubes[:, None, None, ...]
    pillars_nodes = collect_pillars_nodes(two_cubes_grid)
    assert vertical_pillars(pillars_nodes), "pillars should be vertical"
    ...
    assert np.all(test == collect_pillars_nodes(two_cubes_grid))
    # print(test)
    # print(pillars_nodes)
    assert vertical_pillars(pillars_nodes), "pillars should be vertical"
    stairs = four_cells_stairs()
    buffered = add_buffer(stairs)
    pillars_nodes = collect_pillars_nodes(stairs)
    assert vertical_pillars(pillars_nodes), "pillars should be vertical"
    # print('*'*20)
    # print(stairs)
    # print(pillars_nodes)
    # print(pillars(stairs))
